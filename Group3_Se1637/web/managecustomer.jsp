                    ...
                    <div class="panel panel-default">
                        <div class="panel-body">
                            ...
                            <table data-toolbar="#toolbar" data-toggle="table">
                                ...
                                <tbody>
                                    <c:forEach var="i" items="${requestScope.listcus}"> 

                                        <tr data-index="0">
                                            <td style="">${i.getCid()}</td>
                                            <td style="">${i.getUserName()}</td>
                                            <td style="">${i.getName()}</td>
                                            <td style="">${i.getPhone()}</td>
                                            <td style="">${i.getEmail()}</td>
                                            <td>${i.getAddress()}</td>
                                            <td style=""><span class="label label-danger">Customer</span></td>  
                                            ...
                                            <c:if test = "${i.getStatics() == 2}">
                                                <td><span class="label label-warning">Chưa kích hoạt</span></td>
                                            </c:if>
                                            <td class="form-group" style="">
                                                <a href="editinforcus?cid=${i.getCid()}" class="btn btn-primary"><i class="glyphicon glyphicon-pencil"></i></a>

                                                <c:if test = "${i.getStatics() == 0}">
                                                    <a href="unbancus?cid=${i.getCid()}" class="btn btn-danger"><i class="glyphicon glyphicon-eye-open"></i></a>
                                                    </c:if>

                                                <c:if test = "${i.getStatics() == 1}">
                                                    <a href="bancus?cid=${i.getCid()}" class="btn btn-danger"><i class="glyphicon glyphicon-eye-close"></i></a>
                                                    </c:if>
                                                    <c:if test = "${i.getStatics() == 2}">
                                                    <a href="unbancus?cid=${i.getCid()}" class="btn btn-danger"><i class="glyphicon glyphicon-eye-open"></i></a>
                                                    </c:if>
